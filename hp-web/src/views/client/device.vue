<template>
  <div>
    ...
    <a-list
      :loading="listLoading"
      :locale="{ emptyText: '暂无设备,请添加后在客户端' }"
      item-layout="horizontal"
      :data-source="deviceList"
    >
      <template #renderItem="{ item }">
        <div style="padding: 30px" :class="[!item.online ? 'onRead' : '']">
          <a-card :title="item.desc" :bordered="false">
            <p style="overflow-wrap: break-word">设备ID：{{ item.deviceId }}</p>
            <p v-if="item.memoryInfo">
              总内存：{{ (item.memoryInfo.total / 1024 / 1024).toFixed(2) }}/MB
            </p>
            <p v-if="item.memoryInfo">
              使用内存：{{ (item.memoryInfo.useMem / 1024 / 1024).toFixed(2) }}/MB
            </p>
            <p v-if="item.memoryInfo">CPU：{{ item.memoryInfo.cpuRate.toFixed(2) }}%</p>
            <p v-if="item.memoryInfo">
              HP占用内存：{{ (item.memoryInfo.hpTotalMem / 1024 / 1024).toFixed(2) }}/MB
            </p>
            <p v-if="item.memoryInfo">
              HP实际使用内存：{{ (item.memoryInfo.hpUseMem / 1024 / 1024).toFixed(2) }}/MB
            </p>
      ...
      >
        <a-form :model="formState" ref="formTable" :layout="'vertical'">
          ...
          <a-form-item
            label="设备备注"
            name="desc"
            :rules="[{ required: true, message: '设备备注必填' }]"
          >
            <a-input
              style="width: 70%"
              v-model:value="formState.desc"
              placeholder="备注如：nas中的HP"
            />
          </a-form-item>
        </a-form>
      </a-modal>
