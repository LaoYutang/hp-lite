<template>
  <div>
    <a-button type="primary" style="margin-bottom: 10px; margin-left: 10px" @click="addConfigModal">
      添加穿透
    </a-button>
    <a-button type="primary" style="margin-bottom: 10px; margin-left: 10px" @click="loadData">
      刷新列表
    </a-button>
    <a-table
    ...
    >
      <template #bodyCell="{ column, record }">
        <template v-if="column.key === 'deviceKey'">
          <div>设备状态：{{ userKeyByName(record.deviceKey) }}</div>
          <div v-if="userInfo.getUserInfo().role === 'ADMIN'">
            <div v-if="userKeyByUserInfo(record.deviceKey).username">
              归属用户：
        ...
        <template v-if="column.key === 'action'">
          <a-popconfirm
            ...
            ok-text="是"
            cancel-text="否"
            @confirm="removeConfigData(record)"
          >
            <a-button type="primary" style="margin-bottom: 5px; margin-left: 5px"> 删除 </a-button>
          </a-popconfirm>
          <a-button
            type="primary"
            style="margin-bottom: 5px; margin-left: 5px"
            @click="editConfigData(record)"
            >编辑
          </a-button>
          <a-button
            type="primary"
            style="margin-bottom: 5px; margin-left: 5px"
            @click="refConfigData(record)"
            >重连配置
          </a-button>
        </template>
      </template>
        ...
        <div class="text-detail">
          ...
          <div v-if="record.connectType === 'TCP'">
            <div v-if="record.port">
              外网TCP地址: <span class="text-tips">{{ record.serverIp }}:{{ record.port }}</span>
            </div>
            <div v-if="record.domain">
              外网HTTP地址: <span class="text-tips">http(s)://{{ record.domain }}</span>
            </div>
          </div>

          <div v-if="record.connectType === 'UDP'">
            <div v-if="record.port">
              外网UDP地址: <span class="text-tips">{{ record.serverIp }}:{{ record.port }}</span>
            </div>
          </div>
          ...
          <div v-if="!record.port">随机端口不支持TCP和UDP协议使用</div>
        </div>
      </template>
    </a-table>
      ...
      >
        <a-form :model="formState" ref="formTable">
          <a-form-item
            label="穿透设备"
            name="deviceKey"
            :rules="[{ required: true, message: '穿透设备必填' }]"
          >
            <a-select v-model:value="formState.deviceKey" :options="currentUserKeyList"></a-select>
          </a-form-item>

          <a-form-item
            label="穿透备注"
            name="remarks"
            :rules="[{ required: true, message: '穿透备注必填' }]"
          >
          ...
          <a-form-item
            label="穿透协议"
            name="connectType"
            :rules="[{ required: true, message: '穿透协议必填' }]"
          >
            <a-select v-model:value="formState.connectType">
          ...
          <a-form-item
            label="外网端口"
            name="port"
            :rules="[{ required: true, message: '外网端口必填' }]"
          >
          ...
          <a-form-item
            label="内网地址"
            name="localIp"
            :rules="[{ required: true, message: '内网地址必填' }]"
          >
          ...
          <a-form-item
            label="内网端口"
            name="localPort"
            :rules="[{ required: true, message: '内网端口必填' }]"
          >
            <a-input
              v-model:value.number="formState.localPort"
              type="number"
              placeholder="内网端口如：8080"
            />
          </a-form-item>
          ...
          >
            <a-select v-model:value="formState.proxyVersion">
              <a-select-option value="NONE"
                >不设置(小白用户请不要设置-用于获取真实IP)</a-select-option
              >
          ...
          >
            <a-textarea
              disabled="disabled"
              :rows="6"
              v-model:value="formState.certificateKey"
          ...
          >
            <a-textarea
              disabled="disabled"
              :rows="6"
              v-model:value="formState.certificateContent"
...
<script setup>
  ...
  const handleChange = (val) => {
    const data = domainOptions.value.filter((item) => {
